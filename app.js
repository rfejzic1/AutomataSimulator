...
class Node {
  ...
  render() {
    this.links.forEach(node => {
      ...
      this.ctx.stroke();
      this.ctx.lineTo(
        px - 12 * Math.cos(angle - 6),
        py - 12 * Math.sin(angle - 6)
      );
      this.ctx.stroke();
    });
...
class Nodes {
  ...
  addNode(node) {
    if (!node.overlapsAny(this.nodes)) this.nodes.push(node);
    else console.log("There is alredy a Node on that position!");
...
class Simulator {
    ...
    canvas.addEventListener("mousedown", e => {
      ...
      if (e.ctrlKey) {
        ...
        if (this.selected) {
          ...
          this.selected = this.nodesList.selectNode(e.offsetX, e.offsetY);
          if (this.selected) this.nodesList.linkNodes(prev, this.selected);
        }
      } else {
        this.selected = this.nodesList.selectNode(e.offsetX, e.offsetY);
      }
  ...
  drawSelectionBox(x, y) {
    ...
    this.ctx.arc(
      x,
      y,
      this.options.nodeRadius + this.options.selectionBoxOffset,
      0,
      Math.PI * 2,
      true
    );
...
canvas.width =
  ...
  parseInt(getComputedStyle(canvas.parentElement).padding) * 2;
